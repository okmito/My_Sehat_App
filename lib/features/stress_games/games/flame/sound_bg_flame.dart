...
class _ParticleGenerator extends Component
    with HasGameReference<SoundAtmosphereGame> {
  ...
  void _spawnParticle() {
    ...
    switch (mode) {
      case SoundMode.rain:
        startPos = Vector2(_rand.nextDouble() * game.size.x, -10);
        velocity = Vector2(0, 400 + _rand.nextDouble() * 200);
        color = Colors.blueAccent;
        life = game.size.y / velocity.y;
        game.add(
      ...
      case SoundMode.ocean:
        startPos = Vector2(_rand.nextDouble() * game.size.x, game.size.y + 10);
        velocity = Vector2(
      ...
      case SoundMode.wind:
        startPos = Vector2(-10, _rand.nextDouble() * game.size.y);
        velocity = Vector2(
            300 + _rand.nextDouble() * 200, (_rand.nextDouble() - 0.5) * 50);
        color = Colors.grey;
        life = game.size.x / velocity.x;
        game.add(
          ParticleSystemComponent(
            particle: AcceleratedParticle(
              position: startPos,
